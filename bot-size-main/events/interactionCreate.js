...
const { isGerencia, isRegisteredUser, denyNotRegistered } = require('../utils/permissions');
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (interaction.isChatInputCommand()) {
            // Verificar se o usuário está cadastrado no painel
            if (!isRegisteredUser(interaction)) {
                // Log de tentativa de uso não autorizado
                await sendStaffLog(
                    client, 
                    '🚫 Tentativa de Acesso Negada', 
                    `O usuário <@${interaction.user.id}> (\`${interaction.user.id}\`) tentou utilizar o comando \`/${interaction.commandName}\` sem estar cadastrado no painel.`,
                    '#ED4245'
                );
