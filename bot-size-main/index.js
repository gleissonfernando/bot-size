...
const { notifyError, sendUpdateLog, notifyShutdown } = require('./utils/notifications');
...
process.on('uncaughtException', async (error) => {
    ...
    if (client.isReady()) {
        try {
            await notifyError(client, error, 'Uncaught Exception');
            await handleShutdown('EXCEPTION', `Erro Crítico: ${error.message}`);
        } catch {
            process.exit(1);
        }
    } else {
...
const handleShutdown = async (signal, reason = 'Desligamento manual/sistema') => {
    logger.info(`Recebido sinal ${signal}. Iniciando desligamento gracioso...`);
    try {
        if (client && client.isReady()) {
            // Notifica canal e DMs dos devs
            await notifyShutdown(client, reason, signal);
            
            // Aguarda um pouco mais para garantir que as múltiplas promessas (DM + Canal) sejam resolvidas
